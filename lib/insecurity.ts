...
/*
  As chaves privadas foram removidas do código-fonte e passam a ser carregadas
  a partir de variáveis de ambiente, evitando exposição das mesmas.

  Foi adicionada normalização do formato PEM para garantir compatibilidade entre ambientes
  e validação básica de existência das chaves.

  Resultado: reduz risco de exposição de chaves privadas.
             devem ser usadas outras soluções como: AWS Secrets Manager - GCP Secret Manager - Vault
*/
...
const unescapePem = (v?: string): string => (v ?? '').replace(/\\r?\\n/g, '\n')
const privateKey = unescapePem(process.env.JWT_PRIVATE_KEY)  
const publicKey = unescapePem(process.env.JWT_PUBLIC_KEY)
